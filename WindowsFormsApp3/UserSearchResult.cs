...
namespace WindowsFormsApp3
{
    public partial class UserSearchResult : Form
    {
        ...
        public UserSearchResult(int id, string type, string username)
        {
            InitializeComponent();
            LayoutSetup();
            SearchDatabase(id, type, username);
        }
        ...
        public void SearchDatabase(int id, string type, string username)
        {
            ...
            using (SqlConnection connection = new SqlConnection(Main.connectionString))
            {
                ...
                try
                {
                    ...
                    {
                        // Database query
                        string query = $"Select * from BasicCalcInput where ID = {id}";
                    ...
                    else
                    {
                        // Pull data from neccessary different tables, add to dictionary, display at end
                        // Database query
                        string query = $"Select * from AdvCalcInput where ID = {id}";
                        ...
                        advResults.Add("Size Need", row["BTU_Need"].ToString());
